    ...
    However, if you use this no_grad(), you can control the new w1 and new w2 have no gradients since they are generated by operations, which means you only change the value of w1 and w2, not gradient part, they still have previous defined variable gradient information and back propagation can continue
        ...
        for i, (inputs, labels) in enumerate(dataloader):
            ...
            for j in range(inputs.size()[0]):
                images_so_far += 1
                #print(dir(inputs), (num_images//2, 2, images_so_far))
                if images_so_far > num_images:
                    model.train(mode=was_training)
                    break
                imgn = os.path.basename(dataloader.dataset.imgs[j][0])
                if imgn[0] != 'b' and class_names[preds[j]] == 'burned' or \
                    imgn[0] != 'n' and class_names[preds[j]] == 'normal':
        ...
        model.train(mode=was_training)
        misclassified.extend(visualize_model(model, dataloader,device, num_images=num_images)[0])
...
def main():
    ...
    print (images_so_far, len(misclassified), misclassified)
